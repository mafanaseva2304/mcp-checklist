...
from numpy import array, divide, full, ndarray
...
class DNP_model(Model):
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        amounts_substance = array(
            [
                full(input_two_dimen_array[:, 0].shape, 2.1 / 58.12),
                input_two_dimen_array[:, 0] / 58.12,
                input_two_dimen_array[:, 1] / 72.15,
            ]
        )

        substance_content = array([[0.55, 0.41, 0.2], [0.6, 0.45, 0.21]])

        amounts_substance_mol = (
            divide(amounts_substance, amounts_substance.sum(axis=0)) * 100
        )

        delta1 = (amounts_substance_mol.transpose() * substance_content[0] / 100).sum(
            axis=1
        ) - 0.1
        delta2 = (amounts_substance_mol.transpose() * substance_content[1] / 100).sum(
            axis=1
        ) - 0.5

        rizb = (
            (0.1 + (0.5 - 0.1) * delta1 / (delta1 - delta2)) - 0.1
        ) + self.intercept_
        return rizb.tolist()
