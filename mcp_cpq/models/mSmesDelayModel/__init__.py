...
from numpy import isfinite, nan_to_num, ndarray, ones_like
...
class SmesUILN(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if f1[b2_time] + f2[a1_time] > 0:
            b2_mix = (q1[b2_time] * f1[b2_time] + q2[a1_time] * f2[a1_time]) / (
                f1[b2_time] + f2[a1_time]
            )
        else:
            b2_mix = 0
        ...
        if f3[c1_time] + f1[b2_time] > 0:
            b4_mix = (q3[c1_time] * f3[c1_time] + b2_mix * f1[b2_time]) / (
                f3[c1_time] + f1[b2_time]
            )
        else:
...
class SmesGO(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        joint_time = self.get_delay_time(
            0, f3 / 60, v_joint
        )  # время смешения потока 85-120 с 120-180

        v_120_180 = (
            (l1 - l2) * (pi * d2**2) / 4
        )  # объем трубы 120-180 на участке в 60 метров
        ...
        if f_120_180[time_120_180] + f3[joint_time] > 0:
            mix = (
                q1[time_120_180] * f_120_180[time_120_180]
                + q2[joint_time] * f3[joint_time]
...
class SmesGFU2С2(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if (
            ...
            mix = (
                q1[time_avt6] * favt6[time_avt6]
                + q2[time_s200] * f3[time_s200]
                + q3[time_s100] * f4[time_s100]
                + q4[time_3511_go] * f5[time_3511_go]
                + q5[time_3511_rif] * f6[time_3511_rif]
            ) / (
                favt6[time_avt6]
                + f3[time_s200]
                + f4[time_s100]
