...
from numpy import nansum, ndarray, power
from pandas import DataFrame, Series

from models import Model


class RVP_ONPZmodel(Model):
    """
    RVP_ONPZ- ДНП, кПА (ОНПЗ)
    """
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if type(data) == DataFrame:
            ...
            d_q_transformed = power(d_q, 1.147)
            d_fr[d_fr < 1] = 0
            weight_sum = nansum(d_q_transformed * d_fr, axis=1) / nansum(d_fr, axis=1)
            return_list = power(weight_sum, 1 / 1.147) + self.intercept_
