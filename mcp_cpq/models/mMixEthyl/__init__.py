...
from numpy import column_stack, isnan, nansum, ndarray, sum
...
class Mix_Ethyl(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if type(data) == DataFrame:
            ...
            lin_moch_df = (
                data.iloc[:, (ind_half_df + self.k) : (2 * ind_half_df)]
                .to_numpy()
                .astype("float64")
            )
            nl_ioch_df = (
                data.iloc[:, (2 * ind_half_df) : (2 * ind_half_df + self.k)]
                .to_numpy()
                .astype("float64")
            )
            lin_ioch_df = (
                data.iloc[:, (2 * ind_half_df + self.k) : (3 * ind_half_df)]
                .to_numpy()
                .astype("float64")
            )
            nl_olef_df = (
                data.iloc[:, (3 * ind_half_df) : (3 * ind_half_df + self.k)]
                .to_numpy()
                .astype("float64")
            )
            lin_olef_df = (
                data.iloc[:, (3 * ind_half_df + self.k) : (4 * ind_half_df)]
                .to_numpy()
                .astype("float64")
            )
            nl_arom_df = (
                data.iloc[:, (4 * ind_half_df) : (4 * ind_half_df + self.k)]
                .to_numpy()
                .astype("float64")
            )
            lin_arom_df = (
                data.iloc[:, (4 * ind_half_df + self.k) :].to_numpy().astype("float64")
            )
        ...
        if self.type_quality == 1:
            k_1 = nansum(
                nl_mass_df * nl_moch_df * (nl_ioch_df - nl_moch_df), axis=1
            ) - (
                nansum(nl_mass_df * nl_moch_df, axis=1)
                * nansum(nl_mass_df * (nl_ioch_df - nl_moch_df), axis=1)
            ) / nansum(nl_mass_df, axis=1)
            k_2 = nansum(nl_mass_df * (nl_olef_df**2), axis=1) - (
                nansum(nl_mass_df * nl_olef_df, axis=1) ** 2
            ) / nansum(nl_mass_df, axis=1)
            k_3 = (
                nansum(nl_mass_df * (nl_arom_df**2), axis=1)
                - (nansum(nl_mass_df * nl_arom_df, axis=1) ** 2)
                / nansum(nl_mass_df, axis=1)
            ) / 100
            ...
            k_1 = nansum(
                nl_mass_df * nl_ioch_df * (nl_ioch_df - nl_moch_df), axis=1
            ) - (
                nansum(nl_mass_df * nl_ioch_df, axis=1)
                * nansum(nl_mass_df * (nl_ioch_df - nl_moch_df), axis=1)
            ) / nansum(nl_mass_df, axis=1)
            k_2 = nansum(nl_mass_df * (nl_olef_df**2), axis=1) - (
                nansum(nl_mass_df * nl_olef_df, axis=1) ** 2
