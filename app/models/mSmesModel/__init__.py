...
class Smes_model(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if type(data) == DataFrame:
            ...
            return_list = ((nansum(d_q * d_fr, axis=1) / nansum(d_q, axis=1)) + self.intercept_).tolist()
        elif type(data) == ndarray:
            ind_half_array = int(data.shape[1] / 2)
            data[:, :ind_half_array][data[:, :ind_half_array] < 1] = 0
            return_list = ((nansum(data[:, ind_half_array:] * data[:, :ind_half_array], axis=1) /
                            nansum(data[:, :ind_half_array], axis=1)) + self.intercept_).tolist()
        return return_list
