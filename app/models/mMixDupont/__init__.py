from numpy import ndarray, nansum, array, sum, isnan, prod
...
class Mix_Dupont(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        P = (nansum(X_check * self.bonus, axis=1) / sum([nansum(lin_mass_df, axis=1),
                                                         nansum(nl_mass_df, axis=1)], axis=0)**2)
        P[isnan(P)] = 0
