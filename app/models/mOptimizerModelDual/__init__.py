...
from app.models.mOptimizerDual import OptimizerMixingDual, Flows
...
class OptimizerModel():
    ...
    def taking_data(self, cur_data: ndarray):
        ...
        if self.target_name > 5:
            ...
            self.current_procent_flow_t_sm2 = array(cur_data[i:i + self.tu_comp_count])
            i += self.tu_comp_count

            self.current_procent_flow_u_sm2 = array(cur_data[i:i + self.tu_comp_count])
            i += self.tu_comp_count
    ...
    def predict(self, data: ndarray):
        ...
        if self.target_name > 5:
            ...
            flows_dict = {'min_flow_volume': self.min_flow_volume, # - ограничения по объемноу количеству (общее на все смесители)
        ...
        optimizermix.load(models = self.smes_models,
                                    dt = self.dt,
