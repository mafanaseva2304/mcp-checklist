    ...
    def predict(self, data: DataFrame):
        ...
        for i_q, dq_i in enumerate(d_q):
            ...
            if len(mix_flows) < 1:
                ...
                result = {i: j for i, j in zip(range(1, 100), interp(range(1, 100), volumes, mix_temps[-1].astype('float64')))}
                percent_out = dict()
                for temp in out_temps:
                    percent_out[temp] = list(result.keys())[
                                            argmin(abs(array(list(result.values())) - temp))] + self.intercepts_[temp]
                if len(out_temps) > 1:
                ...
                percent_out = dict()
                for temp in out_temps:
                    percent_out[temp] = list(smes_mix.keys())[
                                            argmin(abs(array(list(smes_mix.values())) - temp))] + self.intercepts_[temp]
                if len(out_temps) > 1:
