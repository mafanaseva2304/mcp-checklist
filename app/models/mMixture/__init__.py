...
class mMixture(Model):
    ...
    def a1(self, vol):
        return (-0.0000000000380144 * np.power(vol, 6.0) +
                0.0000000056371828 * np.power(vol, 5.0) +
                0.00000018190875 * np.power(vol, 4.0) -
                0.000069095194 * np.power(vol, 3.0) +
                0.0038943365 * np.power(vol, 2.0) - 0.068398325 * vol + 0.9177)

    def b1(self, vol):
        return (0.000000000004134309 * np.power(vol, 6.0) -
                0.0000000001210607 * np.power(vol, 5.0) -
                0.00000014705044 * np.power(vol, 4.0) +
                0.000019454037 * np.power(vol, 3.0) -
                0.00090298931 * np.power(vol, 2.0) + 0.014332337 * vol + 1.019)

    def a(self, vol):
        return (0.85875 - 0.1945 * np.abs(vol - 4.5) +
                0.1945 * np.abs(vol - 5.5) + 0.1076 * np.abs(vol - 19.5) -
                0.1076 * np.abs(vol - 20.5) + 0.07455 * np.abs(vol - 39.5) -
                0.07455 * np.abs(vol - 40.5) - 0.01075 * np.abs(vol - 59.5) +
                0.01075 * np.abs(vol - 60.5) + 0.03925 * np.abs(vol - 79.5) -
                0.03925 * np.abs(vol - 80.5) - 0.0741 * np.abs(vol - 92.5) +
                0.0741 * np.abs(vol - 93.5))

    def b(self, vol):
        return (1.02042 + 0.04855 * np.abs(vol - 4.5) -
                0.04855 * np.abs(vol - 5.5) - 0.0262 * np.abs(vol - 19.5) +
                0.0262 * np.abs(vol - 20.5) - 0.01375 * np.abs(vol - 39.5) +
                0.01375 * np.abs(vol - 40.5) + 0.00275 * np.abs(vol - 59.5) -
                0.00275 * np.abs(vol - 60.5) - 0.0064 * np.abs(vol - 79.5) +
                0.0064 * np.abs(vol - 80.5) + 0.0134 * np.abs(vol - 92.5) -
                0.0134 * np.abs(vol - 93.5))

    def d86_fun(self, temp_tbp, volume):
        return np.exp(np.log((temp_tbp + 273.15) / self.a(volume)) / self.b(volume)) - 273.15
    ...
    def integfun(self, b, c, t, t50):
        return ((b * np.sqrt(2.0) * np.tan((np.sin(
            (t - t50) / c) * np.sqrt(2.0) * 0.5) / b) +
            ((t - t50) / c)) / np.pi + 1.0) / 2.0

    def diffun_s(self, b, s):
        return (
            (1 + np.power(np.tan(0.5 * np.sin(s) * np.sqrt(2.0) / b), 2)) *
            np.cos(s) + 1) / np.pi / 2.0
    ...
    def train(self, d_threshold: Dict[int, float], d_mult: Dict[int, float],
              d_lab: Dict[int, float], d_pred: Dict[int, float]):
