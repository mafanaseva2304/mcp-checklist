...
from app.models.mTechModeClassifier.utils_mode import hysteresis_mode75, mad_z, mad, rolling_mad
    ...
    def predict(self, data: np.ndarray, params: Dict[str, Any], artifacts: Dict[str, Any] = None) -> tuple[np.ndarray, list[str]]:
        ...
        for i in range(len(final_regime)):
            if final_regime[i] == 0:
                ...
                comments.append(f"стабильный режим: дисперсия < {DISP_TH} по [{tags_ok}]")
            elif final_regime[i] == -1:
                reason = []
                if global_outlier[i]:
                    reason.append(f"глобальный выброс: >= {MIN_TAGS_OUTLIER} тегов превышают MAD-Z")
                if group_out[i]:
                    hit_groups = [name for name, g_tags in GROUP_RULES.items()
