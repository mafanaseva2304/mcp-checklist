...
class Smes_cp_model(Model):
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        return_list = (power((nansum(power(input_array[:, ind_half_array:] + 273, (1 / 0.05)) *
                                     input_array[:, :ind_half_array], axis=1) /
                              nansum(input_array[:, :ind_half_array], axis=1)), 0.05) + self.intercept_ - 273).tolist()
        return return_list
