from numpy import ndarray, nansum, array, sum, isnan
from pandas import DataFrame, Series
from typing import Union
from app.models import Model


class Mix_Bonus(Model):
    # тип линейные бонусы
    ...
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if type(data) == DataFrame:
            ...
            nl_quality_df = data.iloc[:, ind_half_df:(ind_half_df + self.k)].to_numpy().astype('float64')
            lin_quality_df = data.iloc[:, (ind_half_df + self.k):].to_numpy().astype('float64')
        ...
        Ox_lin_check = (nansum(lin_quality_df * lin_mass_df, axis=1) / sum([nansum(lin_mass_df, axis=1),
                                                                            nansum(nl_mass_df, axis=1)], axis=0))
        Ox_lin_check[isnan(Ox_lin_check)] = 0
        Ox_nlin_check = (nansum(nl_quality_df * nl_mass_df, axis=1) / sum([nansum(lin_mass_df, axis=1),
                                                                           nansum(nl_mass_df, axis=1)], axis=0))
        Ox_nlin_check[isnan(Ox_nlin_check)] = 0
        P = (nansum(nl_mass_df * self.bonus, axis=1) / sum([nansum(lin_mass_df, axis=1),
                                                            nansum(nl_mass_df, axis=1)], axis=0))
        P[isnan(P)] = 0
    ...
    def load(self, bonus: list, intercept: float):
