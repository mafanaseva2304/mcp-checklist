from numpy import ndarray, nansum, power, exp, log
from pandas import DataFrame, Series
from typing import Union
from app.models import Model


class POR_ONPZmodel(Model):
    """
    POR_ONPZ- Т.Пот.Текуч., °С (ОНПЗ)
    """
    def predict(self, data: Union[DataFrame, ndarray]):
        ...
        if type(data) == DataFrame:
            ...
            d_q_transformed = exp(12.8852*log(d_q/5*9+32+460)-73.0083)
            d_fr[d_fr < 1] = 0
            weight_sum = (nansum(d_q_transformed * d_fr, axis=1) / nansum(d_fr, axis=1))
            return_list = (5/9*(exp((log(weight_sum)+73.0083)/12.8852)-492) + self.intercept_)
