...
from app.models.mPakAnomalyClassifier.utils_anomaly import gbm_predict
...
class AT5001AStrategy(PakAnomalyStrategy):
    def predict(self, data: np.ndarray, params: Dict[str, Any], artifacts: Dict[str, Any]) -> tuple[np.ndarray, list[str]]:
        """
        0 – no anomaly
        ...
        for reg in (1, 2):
            mask = regime == reg
            if mask.any():
                pred[mask] = gbm_predict(artifacts["models"][reg], data[mask][:, feat_idx])

        out = np.zeros(pak.size, np.int8)
        comments = []
        cnt = {1: 0, 2: 0}                                  # счётчики «подряд» по режимам
        thr = artifacts["thr"];  mp = cfg["MIN_PERSIST"]    
