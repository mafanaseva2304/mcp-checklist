...
class Flows:
    ...
    def load(self,
        ...
        if self.tu_mode == 1:
            ...
            self.comp_quality_t = self.comp_quality_t[:,np.where(self.t_flow_status == 1)].reshape((self.comp_quality_t.shape[0],int(self.t_flow_status.sum())))
            self.comp_quality_u = self.comp_quality_u[:,np.where(self.u_flow_status == 1)].reshape((self.comp_quality_u.shape[0],int(self.u_flow_status.sum())))
        ...
        for status, tu_status in zip(self.component_status, self.tu_component_status):
            if status == 0:
                ...
                if tu_status == 0:
                    ...
                    elif self.t_flow_status[i_tu] == 1 and self.u_flow_status[i_tu] == 0:
                        procent_flow_t.append(flows[i])
                        i += 1
                    elif self.t_flow_status[i_tu] == 0 and self.u_flow_status[i_tu] == 1:
    ...
    def tu_comp_q(self, flows: np.array):
        ...
        comp_q = self.base_quality_flow[:,np.where(self.component_status*self.tu_component_status == 1)].reshape((self.base_quality_flow.shape[0],int((self.component_status*self.tu_component_status == 1).sum())))
        
        # print('comp_q, t_flow_status, u_flow_status')
        # print(self.t_flow_status,self.u_flow_status)
        # print(comp_q.shape)
        
        if self.t_flow_status.sum() > 0.0 and self.u_flow_status.sum() > 0.0 and (self.t_flow_status*self.u_flow_status).sum() > 0.0:
            ...
            comp_q = (self.comp_quality_t * procent_flow_t + self.comp_quality_u * procent_flow_u) / (procent_flow_t + procent_flow_u)
                ...
                if status == 0:
                    ...
                    if tu_status == 0:
                        ...
                        if self.t_flow_status[i_tu] == 1 and self.u_flow_status[i_tu] == 1:
                            ...
                            i_flows += 2
                        elif (self.t_flow_status[i_tu] == 1 and self.u_flow_status[i_tu] == 0) or (self.t_flow_status[i_tu] == 0 and self.u_flow_status[i_tu] == 1):
                            # all_flows[i_all_flows] = flows[i_flows]
                            i_flows += 1                            
                        i_all_flows += 1
                        i_tu +=1 
...
class OptimizerMixingDual:
    ...
    def load(self, func_type: str, dt: float = 1, models = [], flows: list = [], tpc_mode: int = 0, tu_mode: int = 1, vars: dict = {}) -> None:
        ...
        for flow in flows:
            ...
            if 'variance' in self.func_type:
                ...
                flow.target_x_flow = flow.build_x(flow.target_flow, flow.target_flow_t, flow.target_flow_u)
            ...
            if 'price' in self.func_type:
                ...
                flow.price_x_flow = flow.build_x(flow.price_flow, flow.price_flow_t, flow.price_flow_u)
    ...
    def constraint_quality_product(self, s_flows: np.array, i = 0, result_status = False) -> list:
        """
        ...
        # print(i, s_flows)
        all_flows =  self.SM_FLOWS[i].get_all_flows(s_flows)*self.SM_FLOWS[i].total_flow/100.0
        ...
        result_quality = (flow_quality * self.SM_FLOWS[i].total_flow * self.dt / 60.0 +  self.SM_FLOWS[i].tank_quality *  self.SM_FLOWS[i].tank_volume) / ( self.SM_FLOWS[i].total_flow * self.dt / 60.0 + self.SM_FLOWS[i].tank_volume)
    ...
    def constraint_quality_product_sm2(self, flows: np.array, result_status = False) -> list:
        """
    ...
    def constraint_total_flow_limit(self, flows: np.array):
        ...
        for i,s_flows in enumerate(separated_flows): 
            if len(s_flows) > 0:
                all_flows = self.SM_FLOWS[i].get_all_flows(s_flows)
                procent_flow_t, procent_flow_u = self.SM_FLOWS[i].extract_tu_flows(s_flows)
                tu_flows = self.build_real_tu(self.SM_FLOWS[i].t_flow_status,self.SM_FLOWS[i].u_flow_status,procent_flow_t, procent_flow_u)
    ...
    def optimize(self) -> np.array:
        ...
        for objective_func_idx, _ in enumerate(self.func_type):
            ...
            if self.tu_mode == 0:
                self.min_flow_percent_constraint = np.vstack([flow.min_flow_percent.copy() for flow in self.SM_FLOWS])
                self.max_flow_percent_constraint = np.vstack([flow.max_flow_percent.copy() for flow in self.SM_FLOWS])
                ...
                self.min_flow_percent_constraint =  [flow.build_x(flow.min_flow_percent.copy(), flow.min_flow_percent_t.copy(), flow.min_flow_percent_u.copy()) for flow in self.SM_FLOWS]
                self.max_flow_percent_constraint =  [flow.build_x(flow.max_flow_percent.copy(), flow.max_flow_percent_t.copy(), flow.max_flow_percent_u.copy()) for flow in self.SM_FLOWS]
                # print(self.min_flow_percent_constraint)
            self.limit_len_separated_flows = self.min_flow_percent_constraint[0].shape[0]
            ...
            constraint_quality_product = NonlinearConstraint(self.constraint_quality_product_sm2  , 
            ...
            initial_status_quality_product = np.vstack([np.ones(flow.min_quality_product.shape) for flow in self.SM_FLOWS])
            
            initial_tu_status = np.vstack([np.hstack([np.array([i_t,i_u]) for i_t,i_u in zip(flow.t_flow_status, flow.u_flow_status)]) for flow in self.SM_FLOWS])
            ...
            total_tu_status = np.vstack([np.hstack([flow.t_flow_status, flow.u_flow_status]) for flow in self.SM_FLOWS])
            self.initial_status_flow =  np.vstack([np.hstack([flow.component_status*(1 - flow.tu_component_status), total_tu_status[idx]]) for idx, flow in enumerate(self.SM_FLOWS)]).any(axis=0)
            ...
            constraint_flow_limit = NonlinearConstraint(self.constraint_flow_limit,  
            ...
            if len(self.SM_FLOWS)>1:
                ...
                if self.tu_mode == 0:
                    ...
                    self.min_flow_volume_constraint = np.hstack([self.min_flow_volume.copy(), self.min_flow_volume_t.copy(), self.min_flow_volume_u.copy()])[np.where(self.initial_status_flow)]
                    self.max_flow_volume_constraint = np.hstack([self.max_flow_volume.copy(), self.max_flow_volume_t.copy(), self.max_flow_volume_u.copy()])[np.where(self.initial_status_flow)]

                constraint_total_flow_limit = NonlinearConstraint(self.constraint_total_flow_limit,
                                                                  self.min_flow_volume_constraint.copy(),
                                                                  self.max_flow_volume_constraint.copy())
                
                constraint_total_flow_sum = NonlinearConstraint(self.constraint_total_flow_sum,
                                                                np.hstack([flow.total_flow for flow in self.SM_FLOWS]),
                                                                np.hstack([flow.total_flow for flow in self.SM_FLOWS]))

                opt_cond.extend([constraint_total_flow_limit, constraint_total_flow_sum])  


            res_x = np.hstack([flow.build_x(flow.current_flow.copy(), flow.current_procent_flow_t.copy(), flow.current_procent_flow_u.copy()) for flow in self.SM_FLOWS])
            ...
            if not res.success:
                ...
                for i, res_sm in enumerate(separated_res):
                    if len(res_sm) > 0:
                        if  not ((self.constraint_quality_product(res_sm,i) > self.SM_FLOWS[i].min_quality_product).all() and (self.constraint_quality_product(res_sm,i) < self.SM_FLOWS[i].max_quality_product).all()):
                            logger.warning(f"Не выполняются ограничения (строгое неравенство) по качеству продукта {i+1}-го смесителя\n"+
                        ...
                        if  not ((self.constraint_flow_limit(res_sm) > self.min_flow_percent_constraint[i]).all() and (self.constraint_flow_limit(res_sm) < self.max_flow_percent_constraint[i]).all()):
                            logger.warning(f"Не выполняются ограничения (строгое неравенство) по расходам компонентов {i+1}-го смесителя\n"+
                        ...
                        return_tu_flow = self.build_real_tu(self.SM_FLOWS[i].t_flow_status,self.SM_FLOWS[i].u_flow_status,result_flow_t, result_flow_u)
                        result_flow_t_min, result_flow_u_min = self.SM_FLOWS[i].extract_tu_flows(self.constraint_flow_limit(res_sm) > self.min_flow_percent_constraint[i])
                        return_tu_flow_min = self.build_real_tu(self.SM_FLOWS[i].t_flow_status,self.SM_FLOWS[i].u_flow_status, result_flow_t_min, result_flow_u_min, True)
                        result_flow_t_max, result_flow_u_max = self.SM_FLOWS[i].extract_tu_flows(self.constraint_flow_limit(res_sm) < self.max_flow_percent_constraint[i])
                        return_tu_flow_max = self.build_real_tu(self.SM_FLOWS[i].t_flow_status,self.SM_FLOWS[i].u_flow_status, result_flow_t_max, result_flow_u_max, True)
                        result_component_flow_min = self.SM_FLOWS[i].get_all_component_status(self.constraint_flow_limit(res_sm) > self.min_flow_percent_constraint[i])
                        result_component_flow_max = self.SM_FLOWS[i].get_all_component_status(self.constraint_flow_limit(res_sm) < self.max_flow_percent_constraint[i])
                        result_status_flow_min = np.hstack([result_component_flow_min,return_tu_flow_min])
                        result_status_flow_max = np.hstack([result_component_flow_max,return_tu_flow_max])
                        ...
                        result_status_quality_min = self.func_post_status(initial_status_quality_product[i].copy(), self.constraint_quality_product(res_sm,i) > self.SM_FLOWS[i].min_quality_product, self.SM_FLOWS[i].qp_status)
                        result_status_quality_max = self.func_post_status(initial_status_quality_product[i].copy(), self.constraint_quality_product(res_sm,i) < self.SM_FLOWS[i].max_quality_product, self.SM_FLOWS[i].qp_status)
                        ...
                        result.append({'status': objective_func_idx, 
                                    'result_flow_component': self.SM_FLOWS[i].get_all_flows(flows = res_sm),
                ...
                if len(self.SM_FLOWS)>1:
                    ...
                    result_status_flow_volume_min = self.func_post_status(np.ones(len(self.initial_status_flow)), result_total_volume >  self.min_flow_volume_constraint, self.initial_status_flow)
                    result_status_flow_volume_max = self.func_post_status(np.ones(len(self.initial_status_flow)), result_total_volume <  self.max_flow_volume_constraint, self.initial_status_flow)
                    
                    if  not ((result_total_volume > self.min_flow_volume_constraint).all() and (result_total_volume < self.max_flow_volume_constraint).all()):
                            logger.warning(f"Не выполняются ограничения (строгое неравенство) по суммарным расходам компонентов\n"+
        ...
        for i, res_sm in enumerate(separated_res):
            if len(res_sm) > 0:
                logger.warning(f"Выполняются ограничения (строгое неравенство) по качеству продукта {i+1}-го смесителя\n"+
                                            f"quality_product: {self.constraint_quality_product(res_sm,i,result_status=True)}\n"+
                                            f"min_quality: {self.SM_FLOWS[i].min_quality_product}\n"+
                                            f"min_quality_product: {self.constraint_quality_product(res_sm,i) > self.SM_FLOWS[i].min_quality_product}\n"+
                                            f"max_quality_product: {self.constraint_quality_product(res_sm,i) < self.SM_FLOWS[i].max_quality_product}\n"+
                                            f"max_quality: {self.SM_FLOWS[i].max_quality_product}")
                logger.warning(f"Выполняются ограничения (строгое неравенство) по расходам компонентов {i+1}-го смесителя\n"+
                                        f"flow: {self.constraint_flow_limit(res_sm)}\n"+
                                        f"min_percent: {self.min_flow_percent_constraint[i]}\n"+
                                        f"min_flow_percent: {self.constraint_flow_limit(res_sm) > self.min_flow_percent_constraint[i]}\n"+
                                        f"max_flow_percent: {self.constraint_flow_limit(res_sm) < self.max_flow_percent_constraint[i]}\n"+
                ...
                result.append({'status': objective_func_idx+1,
                                'result_flow_component': self.SM_FLOWS[i].get_all_flows(flows = res_sm),
                                # 'result_flow_t_component': self.SM_FLOWS[i].extract_tu_flows(res_sm)[0],
                                # 'result_flow_u_component': self.SM_FLOWS[i].extract_tu_flows(res_sm)[1],
                                'result_flow_tu_component': return_tu_flow,
        ...
        if len(self.SM_FLOWS)>1:
            ...
            result_status_flow_volume_min = self.func_post_status(np.ones(len(self.initial_status_flow)), result_total_volume >  self.min_flow_volume_constraint, self.initial_status_flow)
            result_status_flow_volume_max = self.func_post_status(np.ones(len(self.initial_status_flow)), result_total_volume <  self.max_flow_volume_constraint, self.initial_status_flow)
            logger.warning(f"Выполняются ограничения (строгое неравенство) по суммарным расходам компонентов\n"+
                                        f"flow: {result_total_volume}\n"+
                                        f"min_percent: {self.min_flow_volume_constraint}\n"+
                                        f"min_flow_percent: {result_total_volume > self.min_flow_volume_constraint}\n"+
                                        f"max_flow_percent: {result_total_volume < self.max_flow_volume_constraint}\n"+
    ...
    def func_min_off_spec(self, flows: np.array):
        ...
        for i, s_flows in enumerate(separated_flows):
            if len(s_flows) > 0:
                ...
                sum_sm.append(np.sum(np.array(pd_off) * self.SM_FLOWS[i].or_cost[self.SM_FLOWS[i].qp_status==1]))          
        return sum(sum_sm)
    ...
    def func_min_giveaway(self, flows: np.array):
        ...
        for i,s_flows in enumerate(separated_flows):
            if len(s_flows) > 0:
                ...
                pd_g = []
                for t, Q_t in enumerate(curr_Q):
                    if Q_t <= self.SM_FLOWS[i].max_quality_product[t] and Q_t >= self.SM_FLOWS[i].min_quality_product[t]:
                        ...
                        pd_g.append(0)
                sum_sm.append(np.sum(np.array(pd_g) * self.SM_FLOWS[i].pc_cost[self.SM_FLOWS[i].qp_status==1]))
        return sum(sum_sm)
