...
from models.mTechModeClassifier.factory import make_strategy


class TechModeClassifier(Model):
    def __init__(self):
        self.unit = None
    ...
    def load(self, pickle_model: bytes):
        """
        {'unit': str, 'params': dict, 'artifacts': dict}
        unit      – ключ, по которому выбирается алгоритм
        **params  – любые параметры алгоритма
        **artifacts  – доп модели требующие сериализации
        """
        state = unpickle(pickle_model)
        if not isinstance(state, dict) or "unit" not in state:
            raise ValueError("Неверный формат для TechModeClassifier.load")
    ...
    def save(self):
        return pickle.dumps(
            {"unit": self.unit, "params": self.params, "artifacts": self.artifacts},
    ...
    def predict(
        ...
        if self.unit is None:
            raise RuntimeError("TechModeClassifier не инициализирован")
