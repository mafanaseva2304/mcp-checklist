...
class GetReservQuality(Model):
    ...
    def predict(self, data_by_predict: np.ndarray):
        ...
        Quality_L[np.isnan(Quality_L)] = np.broadcast_to(
            Quality_data, Quality_L.T.shape
        ).T[np.isnan(Quality_L)]
        Tank_quality_data[np.isnan(Tank_quality_data)] = Quality_data[
            np.isnan(Tank_quality_data)
        ]
        ...
        if Level_data[Volumes[:, 0] >= 1, :].shape[0] != 0:
            ...
            tank_level = np.apply_along_axis(
                self.calcDeltaVolumeTank,
                axis=1,
                arr=Level_data[Volumes[:, 0] >= 1, :],
                reserv=True,
            )

            if self.tank_type_name_ == "РВС":
                data_layers_volume = self.getRVSRerservVolume(
                    tank_level,
                    DataFrame(
                        {
                            "mirror_area": self.mirror_area_,
                            "line_length": self.line_length_,
                        }
                    ),
                )
            elif self.tank_type_name_ == "РШ":
                data_layers_volume = self.getRSHRerservVolume(
                    tank_level, DataFrame({"height": self.height_})
                )
            else:
                data_layers_volume = self.getRGSRerservVolume(
                    tank_level,
                    DataFrame(
                        {"height": self.height_, "line_length": self.line_length_}
                    ),
                )
    ...
    def calcDeltaVolumeTank(self, Level_data, reserv=None) -> float:
        ...
        if isnan(level_water_data) == False:
            if level_water_data:
                level_water = self.convertUnitMeasurement(
                    level_water_data, self.level_water_data_unit_
                )
    ...
    def getRVSRerservVolume(self, level_data, tank_zone_info):
        data_layers_volume = np.zeros(
            [6, len(level_data)]
        )  # Columns : ['L1', 'L2', 'L3', 'Scont1', 'Scont2', 'Scont3']
        data_layers_volume[0] = tank_zone_info.loc[0, "mirror_area"] * level_data
        data_layers_volume[1] = (
            tank_zone_info.loc[1, "mirror_area"] * level_data - data_layers_volume[0]
        )
        data_layers_volume[2] = (
            tank_zone_info.loc[2, "mirror_area"] * level_data
    ...
    def getRSHRerservVolume(self, level_data, tank_zone_info):
        ...
        data_layers_volume[0] = (
            np.pi
            * np.power(level_data, 2)
            * ((tank_zone_info.loc[0, "height"] / 2) - (level_data / 3))
        )
        data_layers_volume[1] = (
            np.pi
            * np.power(level_data, 2)
            * (tank_zone_info.loc[1, "height"] / 2 - level_data / 3)
            - data_layers_volume[0]
        )
        data_layers_volume[2] = (
            np.pi
            * np.power(level_data, 2)
            * (tank_zone_info.loc[2, "height"] / 2 - level_data / 3)
            - data_layers_volume[0]
            - data_layers_volume[1]
        )
    ...
    def getRGSRerservVolume(self, level_data, tank_zone_info):
        ...
        first_group_ind = (level_data > tank_zone_info.loc[0, "height"]) & (
            level_data > tank_zone_info.loc[1, "height"]
        )
        sec_group_ind = (level_data > tank_zone_info.loc[0, "height"]) & ~(
            level_data > tank_zone_info.loc[1, "height"]
        )
        third_group_ind = ~(level_data > tank_zone_info.loc[0, "height"])

        data_layers_volume[0, first_group_ind] = tank_zone_info.loc[
            0, "line_length"
        ] * ((np.pi * np.power(tank_zone_info.loc[0, "height"], 2)) / 4)
        data_layers_volume[1, first_group_ind] = (
            tank_zone_info.loc[1, "line_length"]
            * ((np.pi * np.power(tank_zone_info.loc[1, "height"], 2)) / 4)
            - data_layers_volume[0, first_group_ind]
        )

        data_layers_volume[0, sec_group_ind] = tank_zone_info.loc[0, "line_length"] * (
            (np.pi * np.power(tank_zone_info.loc[0, "height"], 2)) / 4
        )
        data_layers_volume[1, sec_group_ind] = (
            ...
            * (
                ...
                * np.sqrt(
                    tank_zone_info.loc[1, "height"] * level_data[sec_group_ind]
                    - np.power(level_data[sec_group_ind], 2)
                )
            )
            - data_layers_volume[0, sec_group_ind]
        )

        data_layers_volume[0, third_group_ind] = tank_zone_info.loc[
            0, "line_length"
        ] * (
            ...
            * np.sqrt(
                tank_zone_info.loc[0, "height"] * level_data[third_group_ind]
                - np.power(level_data[third_group_ind], 2)
            )
        )
        data_layers_volume[1, third_group_ind] = (
            ...
            * (
                ...
                * np.arccos(
                    1
                    - 2 * level_data[third_group_ind] / tank_zone_info.loc[1, "height"]
                )
                - (tank_zone_info.loc[1, "height"] / 2 - level_data[third_group_ind])
                * np.sqrt(
                    tank_zone_info.loc[1, "height"] * level_data[third_group_ind]
                    - np.power(level_data[third_group_ind], 2)
                )
            )
        ...
        data_layers_volume[2] = (
            ...
            * (
                ...
                * np.sqrt(
                    tank_zone_info.loc[2, "height"] * level_data
                    - np.power(level_data, 2)
                )
            )
            - data_layers_volume[0]
        ...
        data_layers_volume[3, first_group_ind] = (
            2 * np.pi * (np.power(tank_zone_info.loc[0, "height"], 2))
        ) / 4 + tank_zone_info.loc[0, "line_length"] * np.pi * tank_zone_info.loc[
            0, "height"
        ]
        data_layers_volume[4, first_group_ind] = (
            2 * np.pi * (np.power(tank_zone_info.loc[1, "height"], 2))
        ) / 4 + tank_zone_info.loc[1, "line_length"] * np.pi * tank_zone_info.loc[
            1, "height"
        ]

        data_layers_volume[3, sec_group_ind] = (
            2 * np.pi * (np.power(tank_zone_info.loc[0, "height"], 2))
        ) / 4 + tank_zone_info.loc[0, "line_length"] * np.pi * tank_zone_info.loc[
            0, "height"
        ]
        ...
        ) + tank_zone_info.loc[1, "line_length"] * tank_zone_info.loc[
            1, "height"
        ] * np.arccos(
            1 - 2 * level_data[sec_group_ind] / tank_zone_info.loc[1, "height"]
        )
        ...
        ) + tank_zone_info.loc[0, "line_length"] * tank_zone_info.loc[
            0, "height"
        ] * np.arccos(
            1 - 2 * level_data[third_group_ind] / tank_zone_info.loc[0, "height"]
        )
        ...
        data_layers_volume[5] = 2 * (
            ...
            * np.arccos(1 - 2 * level_data / tank_zone_info.loc[2, "height"])
            - (tank_zone_info.loc[2, "height"] / 2 - level_data)
            * np.sqrt(
                (tank_zone_info.loc[2, "height"]) * level_data - np.power(level_data, 2)
            )
        ) + tank_zone_info.loc[2, "line_length"] * tank_zone_info.loc[
            2, "height"
        ] * np.arccos(
    ...
    def getQualityByZones(self, data_layers_volume, Volumes, Quality_data, Quality_L):
        data_quality = np.zeros(
            [Volumes.shape[0], 4]
        )  # Columns : ['VFnxt2', 'VFnxt3', 'VFprvs1', 'VFprvs2']
        Quality_ind = np.zeros(
            [Volumes.shape[0], 4]
        )  # Columns : ['L1', 'L2', 'L3', 'quality']

        Flow_coefficient = np.full(Volumes.shape[0], self.flow_coefficient_)
        Flow_coefficient[np.any(Volumes == 0, axis=1)] = 0

        VFlow = (
            (Volumes[:, 0] - Volumes[:, 1] * Flow_coefficient)
            / data_layers_volume[0, :]
            * dt
            / 60
        )
        ...
        data_quality[VFlow <= 1, 0] = (
            self.mass_coefficient_ * data_layers_volume[3, VFlow <= 1]
        ) / data_layers_volume[0, VFlow <= 1]
        data_quality[VFlow <= 1, 1] = (
            self.mass_coefficient_ * data_layers_volume[4, VFlow <= 1]
        ) / data_layers_volume[1, VFlow <= 1]
        data_quality[VFlow <= 1, 2] = (
            self.mass_coefficient_ * data_layers_volume[3, VFlow <= 1]
        ) / data_layers_volume[1, VFlow <= 1]
        data_quality[VFlow <= 1, 3] = (
            self.mass_coefficient_ * data_layers_volume[4, VFlow <= 1]
        ) / data_layers_volume[2, VFlow <= 1]
        ...
        for ind in range(data_quality.shape[0]):
            ...
            Quality_ind[ind, 2] = Quality_L[ind, 1] * data_quality[ind, 3] + Quality_L[
                ind, 2
            ] * (1 - data_quality[ind, 3])
            Quality_ind[ind, 3] = Quality_data[ind] * Flow_coefficient[ind] + Quality_L[
                ind, 0
    ...
    def load(
        ...
        mirror_area: np.array,
        flow_coefficient: float,
        mass_coefficient: float,
        calc_formula: str,
        level_data_unit: str,
        level_water_data_unit: str,
        intercept: float = 0,
    ):
        """
