...
class OptimizerMixing:
    ...
    def optimize(self) -> np.array:
        ...
        constraint_target_product = NonlinearConstraint(
            self.constraint_target_product, self.target_product, self.target_product
        )
        ...
        initial_tu_product = np.hstack(
            [self.current_procent_flow_t.copy(), self.current_procent_flow_u.copy()]
        )
        initial_status_quality_product = np.ones(self.min_quality_product.shape)
        initial_status_flow = np.hstack(
            [
                self.component_status * (1 - self.tu_component_status),
                np.ones(initial_tu_product.shape),
            ]
        )
        ...
        if self.tu_mode == 0:
            ...
            self.min_flow_constraint = self.build_x(
                self.min_flow, self.min_flow_t, self.min_flow_u
            )
            self.max_flow_constraint = self.build_x(
                self.max_flow, self.max_flow_t, self.max_flow_u
            )
        constraint_flow_limit = NonlinearConstraint(
            self.constraint_flow_limit,
            self.min_flow_constraint,
        ...
        res_x = self.build_x(
            self.current_flow, self.current_procent_flow_t, self.current_procent_flow_u
        )

        current_quality = self.constraint_quality_product(res_x)
        if (current_quality <= self.max_quality_product).all() and (
            current_quality >= self.min_quality_product
        ).all():
        ...
        for i, _ in enumerate(self.func_type):
            ...
            if not res.success:
                ...
                if not (
                    (
                        self.constraint_quality_product(res.x)
                        > self.min_quality_product
                    ).all()
                    and (
                        self.constraint_quality_product(res.x)
                    ...
                    logger.warning(
                        "Не выполняются ограничения (строгое неравенство) по качеству продукта\n"
                ...
                if not (
                    (self.constraint_flow_limit(res.x) > self.min_flow_constraint).all()
                    and (
                        self.constraint_flow_limit(res.x) < self.max_flow_constraint
                    ).all()
                ):
                    logger.warning(
                        "Не выполняются ограничения (строгое неравенство) по расходам компонентов\n"
                ...
                if i == 0:
                    result_status_quality_min = self.func_post_status(
                        initial_status_quality_product.copy(),
                        self.constraint_quality_product(res.x)
                        > self.min_quality_product,
                        self.qp_status,
                    )
                    ...
                    result_status_flow_min = self.func_post_status(
                        initial_true_status_flow.copy(),
                        self.constraint_flow_limit(res.x) > self.min_flow_constraint,
                        initial_status_flow,
                    )
                    result_status_flow_max = self.func_post_status(
                        initial_true_status_flow.copy(),
                        self.constraint_flow_limit(res.x) < self.max_flow_constraint,
                        initial_status_flow,
                    )
                ...
                result_status_quality_min = self.func_post_status(
                    initial_status_quality_product.copy(),
                    self.constraint_quality_product(res.x) > self.min_quality_product,
                    self.qp_status,
                )
                result_status_quality_max = self.func_post_status(
                    initial_status_quality_product.copy(),
                    self.constraint_quality_product(res.x) < self.max_quality_product,
                    self.qp_status,
                )
                result_status_flow_min = self.func_post_status(
                    initial_true_status_flow.copy(),
                ...
                return np.hstack(
                    [
                        i,
                        np.concatenate(
                            (self.get_all_flows(res_x), *self.extract_tu_flows(res_x))
                        ),
                        result_status_quality_min,
                        result_status_quality_max,
                        result_status_flow_min,
            ...
            flow_quality.append(
                model.predict(
                    np.concatenate(
                        [
                            np.array([all_flows]),
                            np.array([self.base_quality_flow[index]]),
                        ],
                        axis=1,
                    )
                )[0]
    ...
    def func_min_giveaway(self, flows: np.array):
        ...
        for t, Q_t in enumerate(curr_Q):
            if (
                Q_t <= self.max_quality_product[t]
                and Q_t >= self.min_quality_product[t]
            ):
    ...
    def tu_comp_q(self, flows: np.array):
        ...
        comp_q = (
            self.comp_quality_t * procent_flow_t + self.comp_quality_u * procent_flow_u
    ...
    def constraint_quality_product(self, flows: np.array) -> list:
        ...
        flow_quality = np.array(flow_quality)
        result_quality = (
            flow_quality * self.total_flow * self.dt / 60.0
            + self.tank_quality * self.tank_volume
    ...
    def load(
        ...
        if "variance" in self.func_type:
            ...
            self.target_x_flow = self.build_x(
                self.target_flow, self.target_flow_t, self.target_flow_u
            )
        ...
        if "price" in self.func_type:
            ...
            self.price_x_flow = self.build_x(
                self.price_flow, self.price_flow_t, self.price_flow_u
            )
