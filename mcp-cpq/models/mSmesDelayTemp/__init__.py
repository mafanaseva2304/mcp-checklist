...
class SmesTypeModelsPredict:
    def predict(self, data_by_predict: np.ndarray):
        ...
        if "SmesTempUILN" in self.model_.__str__():
            ...
            temps_np[:, 0, :] = data_by_predict[
                :, 0:5
            ]  # 'FQIR0039_Tnk': 'FQIR0039_Tkk'
            temps_np[:, 1, :] = data_by_predict[:, 5:10]  # 'FIC2540_Tnk': 'FIC2540_Tkk'
            temps_np[:, 2, :] = data_by_predict[:, 10:15]  # 'FI1162_Tnk': 'FI1162_Tkk'
            flows_np = data_by_predict[
                :, 15:
            ].T  # 'AVT6:FQI0039_V.F_Model':'KUPN:U100.FIC1162.PV_Model'
            ...
            temps_np[:, 0, :] = data_by_predict[
                :, 0:5
            ]  # 'FQIR0025_Tnk' : 'FQIR0025_Tkk'
            temps_np[:, 1, :] = data_by_predict[
                :, 5:10
            ]  # 'FQIR9947_Tnk' : 'FQIR9947_Tkk'
            flows_np = data_by_predict[
                :, 10:
            ].T  # 'AVT6:FQIR9946.F_Model':'35-11-1000:FR401.F_Model'
            pred = self.model_.predict(temps_np, flows_np)
            ...
            flows_np = data_by_predict[
                :, 15:
            ].T  # 'AVT6:FQIR9946.F_Model':'FI2515_Flow'
            pred = self.model_.predict(temps_np, flows_np)
